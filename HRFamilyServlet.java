...
/**
 * Servlet implementation class HRFamilyServlet
 */
@WebServlet("/HRFamilyServlet")
public class HRFamilyServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public HRFamilyServlet() {
        super();
...
try {
			...
			HRfamily family=new HRfamily(fathername,fatherdob,fathercontact,mothername,motherdob,mothercontact,spousename,spousedatebirth,spousecontact,occupation,placebirth,spouseemail,email);
			HRfamilyDAO familydao=new HRfamilyDAO();
			
			
			if(familydao.update(family))
			{
				out.println("<script>alert('Family Details Updated Successfully.')</script>");
				RequestDispatcher rd=request.getRequestDispatcher("hrfamily.jsp");
